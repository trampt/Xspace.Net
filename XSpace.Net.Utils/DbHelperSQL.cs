...
namespace XSpace.Net.Utils
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        //            }
        //            string oraConnectionString = connectionString;
        ...
        public static int Authority(string url, string role)
        {
            int Permission=-1;
            string sql = "select Permission from UserRolePermission where UserRole='" + role + "'" + " and ObjectID in (select ObjectID from Objects where ObjectURL='" + url + "')";
            DataTable table = DbHelperSQL.Query(sql).Tables[0];
            if (table.Rows.Count > 0)
            {
                Permission = table.Rows[0]["Permission"] ==System.DBNull.Value? -1 : Convert.ToInt32(table.Rows[0]["Permission"]);
