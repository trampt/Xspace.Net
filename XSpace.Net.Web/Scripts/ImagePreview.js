...
//透明图片
ImagePreview.TRANSPARENT = "/images/2-2.png";
...
ImagePreview.prototype = {
  ...
  _remoteData: function () {
      ...
      if (window.createObjectURL != undefined) { // basic
          url = window.createObjectURL(this.file.files[0]);
      } else if (window.URL != undefined) { // mozilla(firefox)
          url = window.URL.createObjectURL(this.file.files[0]);
      } else if (window.webkitURL != undefined) { // webkit or chrome
          url = window.webkitURL.createObjectURL(this.file.files[0]);
      }
